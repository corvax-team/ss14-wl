...
namespace Content.Client.Lobby.UI
{
    ...
    {
        public readonly SkillPrototype Skill;
        public readonly Control Label;
        public readonly RadioOptions<SkillLevel> Options;
        public readonly JobPrototype Job;
        public readonly SkillsConfigurationPrototype Config;
        public readonly SkillLimitation? Limitation;

        public readonly Dictionary<SkillLevel, Button> Buttons;

        public event Action<RadioOptionItemSelectedEventArgs<SkillLevel>>? OnSelected;

        public int Points => GetPoints(Selected);
        public SkillLevel Selected => Options.SelectedValue;

        public SkillSelector(
            SkillPrototype skillproto,
            JobPrototype jobProto,
            SkillsConfigurationPrototype config,
            Control label)
        {
            Job = jobProto;
            Skill = skillproto;
            ...
            Skill.JobLimitations.TryGetValue(Job.ID, out var limitation);
            Limitation = limitation;

            label.Margin = new(0, 0, 10, 0);
            ...
            var dict = new Dictionary<SkillLevel, Button>();
            var first = (int?) null;
            ...
            foreach (var skill in skills)
            {
                ...
                var button = Options.SelectedButton;

                button.ToolTip = info.Description;

                Buttons.Add(skill, button);

                if (limitation != null &&
                    (skill > limitation.MaxSkillLevel ||
                    skill < limitation.MinSkillLevel))
                {
                    button.Disabled = true;

                    var desc = new StringBuilder("[color=red]Уровень навыка недоступен из-за лорных соображений.[/color]\n\n");
                    desc.Append(button.ToolTip);

                    var tooltip = new Tooltip();
                    tooltip.SetMessage(FormattedMessage.FromMarkupOrThrow(desc.ToString()));

                    button.TooltipSupplier = (args) => tooltip;
                }
            }

            if (limitation == null)
                Options.Select(first!.Value);
            else Options.Select((int) limitation.MinSkillLevel);
            ...
            Options.OnItemSelected += (args) => OnSelected?.Invoke(args);
        }

        public int GetPoints(SkillLevel level)
        {
            var sum = 0;
            foreach (var button in Buttons)
            {
                if (button.Key <= Limitation?.MinSkillLevel || button.Key > level)
                    continue;

                var info = GetInfo(button.Key);
                sum += info?.Points ?? 0;
            }
        ...
        public SkillLevelInfo? GetInfo(SkillLevel level)
        {
            if (!Skill.Info.TryGetValue(level, out var info))
                return null;

            return info;
        }

        public int GetMaxPoints(int? age)
        {
            var points = Job.SkillPoints;

            if (age == null)
                return points;

            foreach (var ageModifier in Config.AgeModifier)
            {
                if (age >= ageModifier.Key)
                    points += ageModifier.Value;
            }

            return points;
        }

        public static int GetChosenPoints(List<SkillSelector>? selectors)
        {
            if (selectors == null)
                return -1;

            return selectors.Sum(s => s.Points);
        }
    }
    //WL-Skills-end
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer
    {
        ...
        //WL-Skills-start
        private readonly Dictionary<string, List<SkillSelector>> _skillSelectors;
        private readonly Dictionary<string, int> _skillJobsIndicies;
        private JobPrototype _skillsChosenJob;
        //WL-Skills-end
            ...
            MarkingManager markings)
        {
            ...
            //WL-Skills-start
            #region Skills
            TabContainer.SetTabTitle(2, "Навыки");
            ...
            _skillSelectors = new();
            _skillsChosenJob = jobs[0];
            _skillJobsIndicies = new();

            for (var indexer = 0; indexer < jobs.Count; indexer++)
            {
                var job = jobs[indexer];

                SkillsJobOptionsButton.AddItem(job.LocalizedName, indexer);
                _skillJobsIndicies.Add(job.ID, indexer);
            }

            SkillsJobOptionsButton.OnItemSelected += (args) =>
            {
                SkillsJobOptionsButton.SelectId(args.Id);

                var job = jobs[args.Id];

                if (Profile != null)
                {
                    Profile.SkillsChosenJob = job.ID;
                    _skillsChosenJob = _prototypeManager.Index<JobPrototype>(job.ID);
                }
        ...
        {
            foreach (var selector in _skillSelectors)
            {
                foreach (var skillLevel in selector.Value)
                {
                    if (Profile?.Skills.TryGetValue(selector.Key, out var dict) == true)
                        if (dict.TryGetValue(skillLevel.Skill.ID, out var level))
                            skillLevel.Options.Select((int) level);
                }
            }

            if (Profile != null && _skillJobsIndicies.TryGetValue(Profile.SkillsChosenJob, out var id))
                SkillsJobOptionsButton.SelectId(id);
        }
        ...
        private void RefreshSkills()
        {
            ...
            var skillProtos = _prototypeManager.EnumeratePrototypes<SkillPrototype>()
            ...
            foreach (var job in jobs)
            {
                ...
                foreach (var skillProto in skillProtos)
                {
                    var skillMarkup = $"[head=3]{skillProto.Name}[/head]";
                    if (skillProto.NameColor != null)
                        skillMarkup = $"[color={skillProto.NameColor.Value.ToHex()}]{skillMarkup}[/color]";
                    ...
                    var skillSelector = new SkillSelector(skillProto, job, config, label);

                    skillSelector.OnSelected += (args) =>
                    {
                        Profile = Profile?.WithSkillLevel(job.ID, skillProto.ID, (SkillLevel) args.Id);
                        skillSelector.Options.Select(args.Id);
                    ...
                    _skillSelectors[job.ID].Add(skillSelector);
            ...
            foreach (var selector in selectors)
            {
                ...
                foreach (var (level, button) in selector.Buttons)
                {
                    ...
                    if (changedButtonPoints > maxPoints)
                    {
                        ...
                        var desc = new StringBuilder(remaining == 0
                            ? "[color=red]Не осталось очков скиллов.[/color]\n\n"
                            : $"[color=red]Не хватает {-remaining} о.с[/color]\n\n");
                        desc.Append(button.ToolTip);
        ...
        public void RefreshJobs()
        {
            ...
            foreach (var department in departments)
            {
                ...
                foreach (var job in jobs)
                {
                    ...
                    subnameSelector.ToolTip = job.LocalizedDescription;
