...
namespace Content.Client.Preferences.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : Control
    {
            ...
            IEntityManager entityManager, IConfigurationManager configurationManager)
        {
            ...
            if (traits.Count > 0)
            {
                foreach (var trait in traits)
                {
                    ...
                    selector.PreferenceChanged += (preference, selector) =>
                    {
                        var chosen = new List<TraitPreferenceSelector>();
                        var unchosen = new List<TraitPreferenceSelector>();

                        foreach (var pref in _traitPreferences)
                        {
                            if (pref.Preference)
                                chosen.Add(pref);
                        ...
                        if (preference)
                        {
                            ...
                            {
                                unchosen.ForEach(x => x.Disabled = true);
                            }
                        }
        ...
        private void UpdateTraitsAmountAndPoints(IEnumerable<TraitPreferenceSelector> chosen, int maxTraits)
        {
            CTraitsAmount.SetMarkup(Loc.GetString("humanoid-profile-editor-traits-amount-second",
                ("chosen", chosen.Count().ToString()),
                ("max", maxTraits.ToString())));

            var sum = chosen.Select(x => x.Points).Sum();
            CTraitsPointsSum.SetMarkup(Loc.GetString("humanoid-profile-editor-traits-points-sum-second",
                ("sum", sum),
                ("color", sum switch
                {
                    0 => "green",
        ...
        private void SaveTraitPreferences()
        {
            var chosen = _traitPreferences
                .Where(x => x.Preference)
                .ToList();

            var isSumIsZero = chosen.Select(trait => trait.Points).Sum() == 0;

            _traitPreferences.ForEach(x =>
            {
                var chosen = x.Preference;
                var profileExist = Profile?.TraitPreferences.Contains(x.Trait.ID) ?? false;

                if (isSumIsZero)
                {
                    if (chosen && !profileExist)
                        Profile = Profile?.WithTraitPreference(x.Trait.ID, true);
                    if (!chosen && profileExist)
                        Profile = Profile?.WithTraitPreference(x.Trait.ID, false);
                }
                else
                {
                    if (chosen && !profileExist)
        ...
        private void UpdateTraitPreferences()
        {
            ...
            var chosenTraits = _traitPreferences.Where(trait => trait.Preference);

            UpdateTraitsAmountAndPoints(chosenTraits, maxTraitsAmount);

            var traitsCounter = 0;
            foreach (var preferenceSelector in _traitPreferences)
            {
                if (preferenceSelector.Preference)
                    traitsCounter++;

                if (traitsCounter > maxTraitsAmount)
                {
                    foreach (var item in _traitPreferences)
                    {
                        item.Preference = false;
                        Profile?.WithTraitPreference(item.Trait.ID, false);
                    }
                ...
                if (preferenceSelector.Preference != preference)
                    preferenceSelector.Preference = preference;
            }
        }
        ...
        private sealed class TraitPreferenceSelector : Control
        {
            ...
            public int Points => Trait.AccumulatedPoints;
            ...
            public bool Preference
            {
                ...
                set
                {
                    ...
                    PreferenceChanged?.Invoke(value, this);
                }
            }
            ...
            public TraitPreferenceSelector(TraitPrototype trait)
            {
                ...
                var pointsString = new Label()
                {
                    Text = Points > 0
                        ? "+" + Points.ToString()
                        : Points.ToString(),
                    FontColorOverride = Points switch
                    {
                        > 0 => Color.Green,
                        0 => Color.Gray,
                        < 0 => Color.Red
                ...
                _checkBox.OnToggled += _ => PreferenceChanged?.Invoke(Preference, this);
                ...
                mainBox.AddChild(_checkBox);
                mainBox.AddChild(name);
                mainBox.AddChild(pointsString);
