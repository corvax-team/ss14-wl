...
using Robust.Shared.Log; // WL-Changes
using Robust.Shared.Map.Components;
using Robust.Shared.Player;

namespace Content.IntegrationTests.Tests.GameRules;

[Ignore("Disabled due to ZombieImmune component conflicts with androids/IPCs")] // WL-Turn-off
[TestFixture]
        ...
        void AssertAntagInitialized(AntagSpecifierPrototype antag, ICommonSession session)
        {
            ...
            foreach (var comp in antag.Components)
            {
                ...
                if (entMan.HasComponent<ZombieImmuneComponent>(ent)
                    && comp.Value.Component is PendingZombieComponent or ZombifyOnDeathComponent or IncurableZombieComponent)
                    continue;
                // WL-Changes-End
